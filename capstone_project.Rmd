...
The following plots show how the predictors affect the response time

```{r pairs plot of data}
library(GGally)
# ggpairs(df, aes(color=Voltage, alpha=0.4))
...
p <- ggplot(df, aes(Voltage, Response.Time_ms))
p <- p + geom_boxplot(outlier.colour = "red", outlier.shape = 1, aes(color=Valve.Rev))
#p <- p + + geom_jitter(width=0.2)
print(p)
```
